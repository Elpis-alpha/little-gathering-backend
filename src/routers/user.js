...
router.get('/api/users/id/:id', async (req, res) => {
  ...
  try {
    ...
    if (!posts) return res.status(404).send({ error: 'Server Error' })

    const comments = await Comment.countDocuments({ owner: _id })

    res.send({ user, posts, comments })
...
router.get('/api/users/email/:email', async (req, res) => {
  ...
  try {
    ...
    const comments = await Comment.countDocuments({ owner: user._id })
...
// Sends post request to upload and update the users profile avatar
